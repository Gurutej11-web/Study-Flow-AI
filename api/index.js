    ...
    messages: [
      {
        role: "system",
        content:
          "You are an expert study assistant. You always respond with valid JSON only — no markdown, no code blocks, no extra text.",
      },
...
app.post("/api/generate", async (req, res) => {
  ...
  if (trimmed.length < 20)
    return res.status(400).json({ error: "Please provide at least 20 characters of study material." });
  if (trimmed.length > MAX_TEXT_LENGTH)
    return res.status(400).json({ error: `Text is too long. Maximum is ${MAX_TEXT_LENGTH.toLocaleString()} characters.` });
  try {
